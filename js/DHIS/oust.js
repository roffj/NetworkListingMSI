...
function SelectionTreeSelection()
{
    ...
    me.getSelectedObj = function() 
    {
		...
		if ( selectedOrganisationUnit.length >= 1 )
		{
			...
			returnObj = { 'name': linkTags.text(), 'id': selectedId, 'parents': selectionTree.getParents( selectedId ) };
		}
...
function SelectionTree()
{
	...
	{
		var treeTag = $('#selectionTree');
		var parentTag = treeTag.find('[id="oustOrgUnit' + id + '"]');
		var parents = [];
		
		while( parentTag.length > 0 && parentTag.attr( "parentId" ) != "undefined" )
		{
			var parentId = parentTag.attr( "parentId" );
			parentId = ( parentId === "undefined" ) ? "" : parentId;
			parents.unshift( { "id" : parentId } );
			parentTag = treeTag.find('[id="oustOrgUnit' + parentId + '"]');
		}
