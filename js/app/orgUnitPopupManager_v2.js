...
function OrgUnitPopupManager( dialogFormTag, mapTag, dataListingTableManager, oProviderNetwork )
{
	...
	me.openForm = function( countryInfo, orgUnitLevel, networkTypeName, mode, ouId, parentId, parentName )
	{
		AppUtil.checkAndRedirect_DHISSession( function()
		{
			// Save Data into 'orgUnitInfo' object and reuse this.
			me.setOrgUnitInfo( me.orgUnitInfo, countryInfo, orgUnitLevel, networkTypeName, ouId, parentId, parentName );
			me.mode = mode;
	...
	me.setOrgUnitInfo = function( orgUnitInfo, countryInfo, orgUnitLevel, networkTypeName, ouId, parentId, parentName )
	{
	...
	me.orgUnitFormSetup = function( orgUnitInfo, button_SaveTag )
	{
		...
		me.orgUnitAddTitleTag.text( orgUnitInfo.orgUnitLevelName + " " + me.mode );
			...
			,buttons: 
			{
				"Save": function() 
				{
					...
					{	
						...
						me.checkDuplicateOU( ouId, name, code, function( json_Data )
						{
							var valid = true;
							...
							{	
								if ( me.mode == _mode_Edit )
								{
									me.editOrgUnitAction( me.orgUnitAddTabTag, $( '#orgUnitInfoDialogForm' ), _mode_Edit + " organisation unit information successfully.");
								}
								else if ( me.mode == "Add" )
								{
									// Actual Create perform
									var orgUnitsJSON = me.generateOrgUnit_MetaData( me.orgUnitAddTabTag, "Add" );
									me.createOrgUnitAction(orgUnitsJSON, $( '#orgUnitInfoDialogForm' ), "Add organisation unit information successfully.");
								}
								else
								{
	...
	me.generateCustomHtmlTabs = function( input_ouId )
	{
		...
		try
		{
			...
			$.each( htmlTabList, function( i_tb, item_tb )
			{
				...
				// 3. run script
				var script = item_tb.script.replace( '[ouid]', ouId ).replace( '[HtmlTabId]', '#' + tabId_withCount );
				eval( script );	
	...
	me.populateOrgUnitData = function( ouData, tableTag )
	{
		...
		tableTag.find( "input,select" ).each( function()
		{
			...
			if( tagClass != "" )
			{
				...
				else if( attrName.startsWith( 'dynamicAttr_' ) )
				{
	...
	me.checkDuplicateOU = function( id, name, code, exeFunc )
	{
		...
		me.checkDuplicateOUName( id, "name", name, function( valid ){
		...
		// Check duplicate code
		me.checkDuplicateOUName( id, "code", code, function( valid ){
	...
	me.checkDuplicateOUName = function( id, propertyName, value, exeFunc )
	{
	...
	*/
	...
	{		
		...
		tableTag.find( "input,select" ).each(function()
		{
			...
			if ( attributeName != "" && attributeName != "ouAdd_id" ) 
			{
				...
				if ( attributeName.startsWith( "dynamicAttr_" ) && value != "" ) 
				{
				...
				// Group set
				else if ( attributeName.startsWith("ougs_") ) 
				{
	...
	this.createOrgUnitAction = function( orgUnitsJSON, dialogFormTag, msg )
	{
		...
		, function( data )
		{
			...
			else
			{
				...
				var ouId = data.response.lastImported;
		...
		, function() {}
		, function() { if ( dialogFormTag !== undefined ) dialogFormTag.unblock(); } 
		);
	};
	...
	me.editOrgUnitAction = function( tableTag, dialogFormTag, msg )
	{
	...
	me.setMapCenter = function( coordinateLatLng, placeInfo )
	{
		if ( me.map !== undefined && coordinateLatLng !== undefined )
		{
			...
			{ 
				...
				if ( me.mapLoaded )
				{
					console.log( 'finish Map Interval - mapLoaded, interval: ' + intervalCount );
				...
				{
					console.log( 'finish Map Interval - intervalCount: ' + intervalCount );
